...
/*
 * Sysapp: movdir – Přesune adresář.
 * Použití: movdir <src> <dst> [-rec]
 */
...
void init(void) {
    ...
    if (argc < 3) {
        aster_api_print_line("Pouziti: movdir <src> <dst> [-rec]");
        return;
    }
...

    {
        ...
        if (aster_api_remove_tree(src) != 0) {
            aster_api_set_color(0x0C, 0x00);
            aster_api_print_line("Chyba remove");
            aster_api_set_color(0x0F, 0x00);
            return;
        }
